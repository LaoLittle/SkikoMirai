...
fn bin_main() -> BinResult<()> {
    ...
    let repeat_int: i16 = parse_opt(matches.value_of("repeat")).map_err(|_| "Invalid repeat count")?.unwrap_or(0);
    let repeat = match repeat_int {
        -1 => Repeat::Finite(0),
        0 => Repeat::Infinite,
        _ => Repeat::Finite(repeat_int as _),
    };
    ...
    let decode_thread = thread::Builder::new().name("decode".to_string()).spawn(move || {
